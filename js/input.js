...
function Input(options)
{
  ...
  this.register_action = function(action_name, default_bound, handler)
  {
    actions[action_name] = {
      bound: default_bound,
      handler: [],
    ...
    {
      this.add_action(action_name, handler)
    }
  }
  this.add_action = function(added_action, handler)
  {
    ...
    action.handler.push(handler)
  }

  this.frame = function()
  {
    $.each(active_actions, function(action, handlers)
    {
  ...
  this.activate_action = function(action)
  {
    ...
    active_actions[action] = $.merge([], actions[action].handler)
  }
...
function ActionGroup(options)
{
  ...
  input.register_action("prev",   key_prev,   self.prev)
  input.register_action("next",   key_next,   self.next)
  input.register_action("select", key_select, self.select)
}
